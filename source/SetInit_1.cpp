...
void SetInit_LD_x1(vector<double>& U_vals)																			// function to calculate the DG coefficients for the initial condition for Landau Damping
{
...
void SetInit_LD_x2(vector<double>& U_vals)																			// function to calculate the DG coefficients for the initial condition for Landau Damping
{
    int i1, i2, j1, j2, j3, k, m1,m2,m3,nt=5;																	// declare i1, i2 (to represent cell (i1,i2) in x-space), j1, j2, j3 (to represent cell (j1,j2,j3) in v-space), k (the index of cell (i,j1,j2,j3) in U), m1, m2, m3 (counters for the Gaussian quadrature in 3D) & nt (the number of points in the quadrature)
    double a=A_amp, c=k_wave;																					// declare a (the amplitude of cosine wave) and set it to A_amp & c (the frequency of the cosine wave) and set it to k_wave
    double tp, tp0, tp5, tmp0, tmp1, tmp2, tmp3, tmp4;															// declare tp, tp0, tmp0, tmp1, tmp2, tmp3, tmp4 (temporary values while calculating the quadrature for the integral w.r.t. v)
    ...
    for(j1=0;j1<Nv;j1++)																						// loop through all the velocity cells
    {
    	for(j2=0;j2<Nv;j2++)
    	{
    		for(j3=0;j3<Nv;j3++)
    		{
    			...
    			{
        			for(i1=0;i1<Nx;i1++)		// NOTE THAT EVERY i2 HERE SHOULD BE i1 BUT NOT WHILE STILL 1D														// loop through the space cells in the x2 direction
        			{
        				k=i1*Nx*size_v +i2*size_v + (j1*Nv*Nv + j2*Nv + j3);																										// calculate the index of cell (i1,i2,j1,j2,j3) in U
        				tp0 = (dx + (sin(c*Gridx((double)i2+0.5)) - sin(c*Gridx((double)i2-0.5)))*a/c)*tmp0/dx;																		// calculate b_7k = (int_Ii (1 + Acos(kx)) dx)*(int_Kj Mw(v)*phi_7k(v) dv) (NOTE: int_(Omega_i) (1 + a*cos(c*x)) dx = dx + (sin(c*x_(i+0.5)) - sin(c*x_(i+0.5)))*(a/c))
        				tp5 = (dx + (sin(c*Gridx((double)i2+0.5)) - sin(c*Gridx((double)i2-0.5)))*a/c)*tmp4/dx;																		// calculate b_(7k+6) = (int_Ii (1 + Acos(kx)) dx)*(int_Kj Mw(v)*phi_(7k+6)(v) dv) (NOTE: int_(Omega_i) (1 + a*cos(c*x)) dx = dx + (sin(c*x_(i+0.5)) - sin(c*x_(i+0.5)))*(a/c))
        				U_vals[7*k+0] = 19*tp0/4. - 15*tp5;																												// calculate the coefficient U[7k]
        				U_vals[7*k+6] = 60*tp5 - 15*tp0;																													// calculate the coefficient U[7k+6]

        				U_vals[7*k+2] = (0.5*(sin(c*Gridx((double)i2+0.5)) + sin(c*Gridx((double)i2-0.5))) + (cos(c*Gridx((double)i2+0.5)) - cos(c*Gridx((double)i2-0.5)))/(c*dx))*(a/c)*tmp0*12./dx;	// calculate the coefficient U[7k+1] = 12*b_(7k+1) = 12*(int_Ii (1 + Acos(kx))*phi_(7k+1)(x) dx)*(int_Kj Mw(v) dv) (NOTE: int_(Omega_i) (1 + a*cos(c*x))*phi_(6k+1)(x) dx = (0.5*(sin(c*x_(i+0.5)) + sin(c*x_(i+0.5))) + (cos(c*x_(i+0.5)) - cos(c*x_(i+0.5)))/(c*dx))*(a/c))
        				U_vals[7*k+3] = (dx + (sin(c*Gridx((double)i2+0.5)) - sin(c*Gridx((double)i2-0.5)))*a/c)*tmp1*12/dx;																// calculate the coefficient U[7k+3] = 12*b_(7k+3) = 12*(int_Ii (1 + Acos(kx)) dx)*(int_Kj Mw(v)*phi_(6k+2)(v) dv) (NOTE: int_(Omega_i) (1 + a*cos(c*x)) dx = dx + (sin(c*x_(i+0.5)) - sin(c*x_(i+0.5)))*(a/c))
        				U_vals[7*k+4] = (dx + (sin(c*Gridx((double)i2+0.5)) - sin(c*Gridx((double)i2-0.5)))*a/c)*tmp2*12/dx;																// calculate the coefficient U[7k+4] = 12*b_(7k+4) = 12*(int_Ii (1 + Acos(kx)) dx)*(int_Kj Mw(v)*phi_(6k+3)(v) dv) (NOTE: int_(Omega_i) (1 + a*cos(c*x)) dx = dx + (sin(c*x_(i+0.5)) - sin(c*x_(i+0.5)))*(a/c))
        				U_vals[7*k+5] = (dx + (sin(c*Gridx((double)i2+0.5)) - sin(c*Gridx((double)i2-0.5)))*a/c)*tmp3*12/dx;																// calculate the coefficient U[7k+5] = 12*b_(7k+5) = 12*(int_Ii (1 + Acos(kx)) dx)*(int_Kj Mw(v)*phi_(6k+4)(v) dv) (NOTE: int_(Omega_i) (1 + a*cos(c*x)) dx = dx + (sin(c*x_(i+0.5)) - sin(c*x_(i+0.5)))*(a/c))
        			}
    			}
    		}
    	}

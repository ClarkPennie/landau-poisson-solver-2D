    	...
    	for(j2=0;j2<Nv;j2++)
    	{
    		for(j3=0;j3<Nv;j3++)
    		{
    			...
    			for(i2=0;i2<Nx;i2++)																																// loop through the space cells in the x1 direction|
    			{
        			...
        			{
        				k=i1*Nx*size_v +i2*size_v + (j1*Nv*Nv + j2*Nv + j3);																										// calculate the index of cell (i1,i2,j1,j2,j3) in U
        				tp0 = (dx + (sin(c*Gridx(i2+0.5)) - sin(c*Gridx(i2-0.5)))*a/c)*tmp0/dx;																		// calculate b_7k = (int_Ii (1 + Acos(kx)) dx)*(int_Kj Mw(v)*phi_7k(v) dv) (NOTE: int_(Omega_i) (1 + a*cos(c*x)) dx = dx + (sin(c*x_(i+0.5)) - sin(c*x_(i+0.5)))*(a/c))
        				tp5 = (dx + (sin(c*Gridx(i2+0.5)) - sin(c*Gridx(i2-0.5)))*a/c)*tmp4/dx;																		// calculate b_(7k+6) = (int_Ii (1 + Acos(kx)) dx)*(int_Kj Mw(v)*phi_(7k+6)(v) dv) (NOTE: int_(Omega_i) (1 + a*cos(c*x)) dx = dx + (sin(c*x_(i+0.5)) - sin(c*x_(i+0.5)))*(a/c))
        				U_vals[7*k+0] = 19*tp0/4. - 15*tp5;																												// calculate the coefficient U[7k]
        				U_vals[7*k+6] = 60*tp5 - 15*tp0;																													// calculate the coefficient U[7k+6]

        				U_vals[7*k+2] = (0.5*(sin(c*Gridx(i2+0.5)) + sin(c*Gridx(i2-0.5))) + (cos(c*Gridx(i2+0.5)) - cos(c*Gridx(i2-0.5)))/(c*dx))*(a/c)*tmp0*12./dx;	// calculate the coefficient U[7k+1] = 12*b_(7k+1) = 12*(int_Ii (1 + Acos(kx))*phi_(7k+1)(x) dx)*(int_Kj Mw(v) dv) (NOTE: int_(Omega_i) (1 + a*cos(c*x))*phi_(6k+1)(x) dx = (0.5*(sin(c*x_(i+0.5)) + sin(c*x_(i+0.5))) + (cos(c*x_(i+0.5)) - cos(c*x_(i+0.5)))/(c*dx))*(a/c))
        				U_vals[7*k+3] = (dx + (sin(c*Gridx(i2+0.5)) - sin(c*Gridx(i2-0.5)))*a/c)*tmp1*12/dx;																// calculate the coefficient U[7k+3] = 12*b_(7k+3) = 12*(int_Ii (1 + Acos(kx)) dx)*(int_Kj Mw(v)*phi_(6k+2)(v) dv) (NOTE: int_(Omega_i) (1 + a*cos(c*x)) dx = dx + (sin(c*x_(i+0.5)) - sin(c*x_(i+0.5)))*(a/c))
        				U_vals[7*k+4] = (dx + (sin(c*Gridx(i2+0.5)) - sin(c*Gridx(i2-0.5)))*a/c)*tmp2*12/dx;																// calculate the coefficient U[7k+4] = 12*b_(7k+4) = 12*(int_Ii (1 + Acos(kx)) dx)*(int_Kj Mw(v)*phi_(6k+3)(v) dv) (NOTE: int_(Omega_i) (1 + a*cos(c*x)) dx = dx + (sin(c*x_(i+0.5)) - sin(c*x_(i+0.5)))*(a/c))
        				U_vals[7*k+5] = (dx + (sin(c*Gridx(i2+0.5)) - sin(c*Gridx(i2-0.5)))*a/c)*tmp3*12/dx;																// calculate the coefficient U[7k+5] = 12*b_(7k+5) = 12*(int_Ii (1 + Acos(kx)) dx)*(int_Kj Mw(v)*phi_(6k+4)(v) dv) (NOTE: int_(Omega_i) (1 + a*cos(c*x)) dx = dx + (sin(c*x_(i+0.5)) - sin(c*x_(i+0.5)))*(a/c))
        			}
    			}
    		}
    	}

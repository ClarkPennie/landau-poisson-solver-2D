...
double rho_x(double x, double *U, int i) // for x in I_i			// MAY NEVER BE CALLED
{
  int j, k;
  double tmp=0.;
...
double rho(double *U, int i) //  \int f(x,v) dxdv in I_i * K_j		// MAY NEVER BE CALLED
{
  int j, k;
  double tmp=0.;
 // #pragma omp parallel for shared(U) reduction(+:tmp)
...
double I1(double *U, int k, int l) // Calculate the first inegtral in H_(i,j), namely \int v1*f*phi_x dxdv
{
  ...
  if(l==1) result = dv*dv*dv*( Gridv((double)j1)*U[k*7+0] + dv*U[k*7+3]/12. + U[k*7+6]*Gridv((double)j1)/4.);
  else result=0.;
...
double I2(double *U, int k, int l) // Calculate the fourth integral in H_(i,j), namely \int E*f*phi_v1 dxdv
{
  ...
  if(l==3) result = Int_fE(U,i2,j_mod)/dv;
  else if(l==6) result = U[k*7+3]*dv*dv*intE[i2]/6.;
  else result = 0.;
...
double I3(double *U, int k, int l) 																			// Calculate the difference of the second and third integrals in H_(i,j), namely \int_j v1*gh*phi dv at interface x=x_i+1/2 - \int_j v1*gh*phi dv at interface x=x_i-1/2
{
	...
	if(j1<Nv/2)																								// do this if j1 < Nv/2 (so that the velocity in the v1 direction is negative)
	{
		...
		ur = -U[kkr*7+1]; 																					// set ur to the negative of the coefficient of the basis function with shape l which is non-zero in the cell with global index kkr (which corresponds to the evaluation of gh^+ at the right boundary and -ve since v_1 < 0 in here)
		ul = -U[kkl*7+1];																					// set ul to the negative of the coefficient of the basis function with shape l which is non-zero in the cell with global index kkl	(which corresponds to the evaluation of gh^+ at the left boundary and -ve since phi < 0 here)
	}
	else																									// do this if j1 >= Nv/2 (so that the velocity in the v1 direction is non-negative)
	{
		...
		ur = U[kkr*7+1];																					// set ur to the value of the coefficient of the basis function with shape l which is non-zero in the cell with global index kkr (which corresponds to the evaluation of gh^- at the right boundary and +ve since v_1 >= 0 in here)
		ul = U[kkl*7+1];																					// set ul to the value of the coefficient of the basis function with shape l which is non-zero in the cell with global index kkl (which corresponds to the evalutaion of gh^- at the left boundary and +ve since v_r >= 0 in here)
	}
  
	if(l==0)result = dv*dv*dv*( (U[kkr*7+0]+0.5*ur - U[kkl*7+0]-0.5*ul)*Gridv((double)j1) + (U[kkr*7+3]-U[kkl*7+3])*dv/12. + (U[kkr*7+6]-U[kkl*7+6])*Gridv((double)j1)/4.);					// calculate \int_j v1*gh*phi dv at interface x=x_i+1/2 - \int_j v1*gh*phi dv at interface x=x_i-1/2 for the basis function with shape 0 (i.e. constant) which is non-zero in the cell with global index k
	if(l==1)result = 0.5*dv*dv*dv*( (U[kkr*7+0]+0.5*ur + U[kkl*7+0]+0.5*ul)*Gridv((double)j1) + (U[kkr*7+3]+U[kkl*7+3])*dv/12. + (U[kkr*7+6]+U[kkl*7+6])*Gridv((double)j1)/4.);				// calculate \int_j v1*gh*phi dv at interface x=x_i+1/2 - \int_j v1*gh*phi dv at interface x=x_i-1/2 for the basis function with shape 1 (i.e. linear in x) which is non-zero in the cell with global index k
	if(l==3)result = dv*dv*(( (U[kkr*7+0]-U[kkl*7+0])*dv*dv + (ur-ul)*0.5*dv*dv + (U[kkr*7+3]-U[kkl*7+3])*dv*Gridv((double)j1))/12. + (U[kkr*7+6]-U[kkl*7+6])*dv*dv*19./720.);				// calculate \int_j v1*gh*phi dv at interface x=x_i+1/2 - \int_j v1*gh*phi dv at interface x=x_i-1/2 for the basis function with shape 2 (i.e. linear in v_1) which is non-zero in the cell with global index k
	if(l==4)result = (U[kkr*7+4]-U[kkl*7+4])*Gridv((double)j1)*dv*dv*dv/12.;																												// calculate \int_j v1*gh*phi dv at interface x=x_i+1/2 - \int_j v1*gh*phi dv at interface x=x_i-1/2 for the basis function with shape 3 (i.e. linear in v_2) which is non-zero in the cell with global index k
	if(l==5)result = (U[kkr*7+5]-U[kkl*7+5])*Gridv((double)j1)*dv*dv*dv/12.;																												// calculate \int_j v1*gh*phi dv at interface x=x_i+1/2 - \int_j v1*gh*phi dv at interface x=x_i-1/2 for the basis function with shape 4 (i.e. linear in v_3) which is non-zero in the cell with global index k
	if(l==6)result = dv*dv*dv*((U[kkr*7+0] + 0.5*ur - U[kkl*7+0]-0.5*ul)*Gridv((double)j1)/4. + (U[kkr*7+3]-U[kkl*7+3])*dv*19./720. + (U[kkr*7+6]-U[kkl*7+6])*Gridv((double)j1)*19./240.);	// calculate \int_j v1*gh*phi dv at interface x=x_i+1/2 - \int_j v1*gh*phi dv at interface x=x_i-1/2 for the basis function with shape 5 (i.e. modulus of v) which is non-zero in the cell with global index k
...
double I5(double *U, int k, int l) 	// Calculate the difference of the fifth and sixth integrals in H_(i,j), namely \int_i E*f*phi dx at interface v1==v_j+1/2 - \int_i E*f*phi dx at interface v1==v_j-1/2
{
	...
	if(intE[i2]>0)																					// do this if the average direction of the field E over the space cell i is positive
	{
		...
		if(j1r<Nv)ur = -U[kkr*7+3];																	// if j1r is not Nv (so that this cell is not receiving information from the right boundary), set ur to the negative of the coefficient of the basis function with shape 2 which is non-zero in the cell with global index kkr (which corresponds to the evaluation of gh^- at the right boundary and -ve since the field is negative here?) - note that if the cell was receiving information from the right boundary then gh^- = 0 here so ur is not needed
		ul = -U[kkl*7+3];																			// set ul to the negative of the coefficient of the basis function with shape 2 which is non-zero in the cell with global index kkl (which corresponds to the evaluation of gh^- at the right boundary and -ve since phi < 0 here)
	}
	else																							// do this if the average direction of the field E over the space cell i is non-positive
	{
		...
		ur = U[kkr*7+3];																			// set ur to the the coefficient of the basis function with shape 2 which is non-zero in the cell with global index kkr (which corresponds to the evaluation of gh^+ at the left boundary and +ve since phi > 0 here)
		if(j1l>-1)ul = U[kkl*7+3];																	// if j1l is not -1 (so that this cell is not receiving information from the left boundary), set ul to the coefficient of the basis function with shape 2 which is non-zero in the cell with global index kkl (which corresponds to the evaluation of gh^+ at the left boundary and +ve since phi < 0 here and being subtracted?) - note that if the cell was receiving information from the left boundary then gh^+ = 0 here so ul is not needed
	}
	...
	{
		if(j1r<Nv && j1l>-1) result = dv*dv*(U[kkr*7+0] + 0.5*ur + U[kkr*7+6]*5./12.- U[kkl*7+0] - 0.5*ul - U[kkl*7+6]*5./12.)*intE[i2] + dv*dv*(U[kkr*7+1]-U[kkl*7+1])*intE1[i2];		// this is the value at an interior cell
		else if(j1r<Nv)result =   dv*dv*(U[kkr*7+0] + 0.5*ur + U[kkr*7+6]*5./12.)*intE[i2] + dv*dv*U[kkr*7+1]*intE1[i2];																	// this is the value at the cell at the left boundary in v1 (so that the integral over the left edge is zero)
		else if(j1l>-1)result = dv*dv*(- U[kkl*7+0] - 0.5*ul - U[kkl*7+6]*5./12.)*intE[i2] - dv*dv*U[kkl*7+1]*intE1[i2];																	// this is the value at the cell at the right boundary in v1 (so that the integral over the right edge is zero)
	}
	...
	{
		if(j1r<Nv && j1l>-1) result = dv*dv*( (U[kkr*7+0] + 0.5*ur + U[kkr*7+6]*5./12. - U[kkl*7+0] - 0.5*ul - U[kkl*7+6]*5./12.)*intE1[i2] + (U[kkr*7+1] - U[kkl*7+1])*intE2[i2] );		// this is the value at an interior cell
		else if(j1r<Nv)result=dv*dv*( (U[kkr*7+0] + 0.5*ur + U[kkr*7+6]*5./12.)*intE1[i2] + U[kkr*7+1]*intE2[i2] );																		// this is the value at the cell at the left boundary in v1 (so that the integral over the left edge is zero)
		else if(j1l>-1)result=dv*dv*( (- U[kkl*7+0] - 0.5*ul - U[kkl*7+6]*5./12.)*intE1[i2] - U[kkl*7+1]*intE2[i2] );																		// this is the value at the cell at the right boundary in v1 (so that the integral over the right edge is zero)
	}
	...
	{
		if(j1r<Nv && j1l>-1)result = 0.5*(dv*dv*(U[kkr*7+0] + 0.5*ur + U[kkr*7+6]*5./12.+ U[kkl*7+0] + 0.5*ul + U[kkl*7+6]*5./12.)*intE[i2] + dv*dv*(U[kkr*7+1]+U[kkl*7+1])*intE1[i2]);	// this is the value at an interior cell
		else if(j1r<Nv)result = 0.5*(dv*dv*(U[kkr*7+0] + 0.5*ur + U[kkr*7+6]*5./12.)*intE[i2] + dv*dv*U[kkr*7+1]*intE1[i2]);																// this is the value at the cell at the left boundary in v1 (so that the integral over the left edge is zero)
		else if(j1l>-1)result = 0.5*(dv*dv*(U[kkl*7+0] + 0.5*ul + U[kkl*7+6]*5./12.)*intE[i2] + dv*dv*U[kkl*7+1]*intE1[i2]);    															// this is the value at the cell at the right boundary in v1 (so that the integral over the right edge is zero)
	}
	...
	{
		if(j1r<Nv && j1l>-1)result = (U[kkr*7+4]-U[kkl*7+4])*intE[i2]*dv*dv/12.;						// this is the value at an interior cell
		else if(j1r<Nv)result = U[kkr*7+4]*intE[i2]*dv*dv/12.;										// this is the value at the cell at the left boundary in v1 (so that the integral over the left edge is zero)
		else if(j1l>-1)result = -U[kkl*7+4]*intE[i2]*dv*dv/12.;										// this is the value at the cell at the right boundary in v1 (so that the integral over the right edge is zero)
	}
	...
	{
		if(j1r<Nv && j1l>-1)result = (U[kkr*7+5]-U[kkl*7+5])*intE[i2]*dv*dv/12.;						// this is the value at an interior cell
		else if(j1r<Nv)result=U[kkr*7+5]*intE[i2]*dv*dv/12.;											// this is the value at the cell at the left boundary in v1 (so that the integral over the left edge is zero)
		else if(j1l>-1)result=-U[kkl*7+5]*intE[i2]*dv*dv/12.;										// this is the value at the cell at the right boundary in v1 (so that the integral over the right edge is zero)
	}
	...
	{
		if(j1r<Nv && j1l>-1)result = dv*dv*( ((U[kkr*7+0] + 0.5*ur - U[kkl*7+0] - 0.5*ul)*5./12. + (U[kkr*7+6]- U[kkl*7+6])*133./720.)*intE[i2] + (U[kkr*7+1] - U[kkl*7+1])*intE1[i2]*5./12. ); //BUG: coefficient of U[k][5] was 11/48 insteadof 133/720		// this is the value at an interior cell
		else if(j1r<Nv)result= dv*dv*( ((U[kkr*7+0] + 0.5*ur)*5./12. + U[kkr*7+6]*133./720.)*intE[i2] + U[kkr*7+1]*intE1[i2]*5./12. );													// this is the value at the cell at the left boundary in v1 (so that the integral over the left edge is zero)
		else if(j1l>-1)result=-dv*dv*( ((U[kkl*7+0] + 0.5*ul)*5./12. + U[kkl*7+6]*133./720.)*intE[i2] + U[kkl*7+1]*intE1[i2]*5./12. );													// this is the value at the cell at the right boundary in v1 (so that the integral over the right edge is zero)
	}
...
void RK3(double *U) // RK3 for f_t = H(f)
{
  ...
  for(k=chunksize_dg*myrank_mpi;k<chunksize_dg*(myrank_mpi+1);k++){
    ...
    for(l=0;l<7;l++) Utmp[k_local*7+l] = U[k*7+l] + dt*H[l];
  } 
  ...
  MPI_Bcast(U1, size*7, MPI_DOUBLE, 0, MPI_COMM_WORLD);
  ...
  for(k=chunksize_dg*myrank_mpi;k<chunksize_dg*(myrank_mpi+1);k++){      
    ...
    tp2=I1(U1,k,3)-I2(U1,k,3)-I3(U1,k,3)+I5(U1,k,3);
    tp3=I1(U1,k,4)-I2(U1,k,4)-I3(U1,k,4)+I5(U1,k,4);
    tp4=I1(U1,k,5)-I2(U1,k,5)-I3(U1,k,5)+I5(U1,k,5);
    tp5=I1(U1,k,6)-I2(U1,k,6)-I3(U1,k,6)+I5(U1,k,6);
    ...
    for(l=0;l<7;l++) Utmp[k_local*7+l] = 0.75*U[k*7+l] + 0.25*U1[k*7+l] + 0.25*dt*H[l];
  }    
  if(myrank_mpi == 0) {
    //dump the weights we've computed into U1
    for(k=0;k<chunksize_dg;k++) {
...
ce = computePhi_x_0(U1); 
  ...
  for(k=chunksize_dg*myrank_mpi;k<chunksize_dg*(myrank_mpi+1);k++){      
    ...
    for(l=0;l<7;l++) Utmp[k_local*7+l] = U[k*7+l]/3. + U1[k*7+l]*2./3. + dt*H[l]*2./3.;
  }    
  if(myrank_mpi == 0) {
    //dump the weights we've computed into U1
    for(k=0;k<chunksize_dg;k++) {
	      for(l=0;l<7;l++) U[k*7+l] = Utmp[k*7+l];
    } 
    //receive from all other processes
    for(i=1;i<nprocs_mpi;i++) {      
	    MPI_Recv(output_buffer_vp, chunksize_dg*7, MPI_DOUBLE, i, i, MPI_COMM_WORLD, &status); // receive weight from other processes consecutively, rank i=1..numNodes-1, ensuring the weights are stored in the file consecutively !
	    for(k=0;k<chunksize_dg;k++) {
		  for(l=0;l<7;l++)U[(k + i*chunksize_dg)*7+l] = output_buffer_vp[k*7+l];
        } 
    }
  }
  else MPI_Send(Utmp, chunksize_dg*7, MPI_DOUBLE, 0, myrank_mpi, MPI_COMM_WORLD);
  
  MPI_Bcast(U, size*7, MPI_DOUBLE, 0, MPI_COMM_WORLD);
  MPI_Barrier(MPI_COMM_WORLD); 
...
void RK3(double *U) // RK3 for f_t = H(f)
{
  ...
  for(k=0;k<size;k++){				  
	  for(l=0;l<7;l++) U1[k*7+l] = U[k*7+l] + dt*H[k*7+l];
  }
 
  computeH(U1);
   #pragma omp parallel for private(k,l) shared(U)
  for(k=0;k<size;k++){						  
	  for(l=0;l<7;l++) U1[k*7+l] = 0.75*U[k*7+l] + 0.25*U1[k*7+l] + 0.25*dt*H[k*7+l];
  }

  computeH(U1);
   #pragma omp parallel for private(k,l) shared(U)
  for(k=0;k<size;k++){						  
	for(l=0;l<7;l++) U[k*7+l] = U[k*7+l]/3. + U1[k*7+l]*2./3. + dt*H[k*7+l]*2./3.;
  }
  //free(H); free(U1);
}
#endif

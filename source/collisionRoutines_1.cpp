#include "collisionRoutines_1.h"																					// collisionRoutines_1.h is where the prototypes for the functions contained in this file are declared and any variables defined here to be used throughout the other files are declared as external

extern fftw_plan p_forward; 
extern fftw_plan p_backward; 
extern fftw_complex *temp;

double IntM[10];						//SHOULD BE IN COLLISIONROUTINES!															// declare an array IntM to hold 10 double variables
#pragma omp threadprivate(IntM)		//SHOULD BE IN COLLISIONROUTINES!																// start the OpenMP parallel construct to start the threads which will run in parallel, passing IntM to each thread as private variables which will have their contents deleted when the threads finish (doesn't seem to be doing anything since no {} afterwards???)

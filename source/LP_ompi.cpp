...
int Nx=32, Nv=24, nT=100, N=16;												 						// declare Nx (no. of x discretised points), Nv (no. of v discretised point), nT (no. of time discretised points) & N (no. of nodes in the spectral method) and setting all their values			// DEBUG: Nx = 5, Nv = 5
...
double scale, scale3, scaleL=8*Lv*Lv*Lv, scalex = dx*dx, scalev=dv*dv*dv;							// declare scale (the 1/sqrt(2pi) factor appearing in Gaussians), scale (the 1/(sqrt(2pi))^3 factor appearing in the Maxwellian), scaleL (the volume of the velocity domain) and set it to 8Lv^3, scalex (the volume of a discretised space element) and set it to dx^2 & scalev (the volume of a discretised velocity element) and set it to dv^3
...
//		chunk_Nx = Nx/nprocs_mpi + 1;																// if nprocs_mpi does not divide into Nx, set chunk_Nx to Nx/nprocs_mpi + 1
	...
	*/
	...
	fEquiVals = (double*)malloc(5*5*5*5*5*size*sizeof(double));										// allocate enough space at the pointer fEquiVals for 5*Nx*(5*Nv)^3 many doubles
	...
	sprintf(buffer_flags,"PerBC_withField");														// store a string in buffer_flags to tag the files for this run
	...
	if(myrank_mpi==0)
	{
	  ...
	  if(myrank_mpi == 0)
	  {
		  for(int i1=0; i1<Nx; i1++)
		  {
			  ...
			  {
				  k = i1*Nx*size_v + i2*size_v + 3*Nv*Nv + 3*Nv + 3;
				  for(l=0; l<7; l++)
				  {
					  printf("i1 = %d, i2 = %d, U[%d*7+%d] = %g \n", i1, i2, k, l, U[k*7+l]);
				  }
			  }
		  }
	  }
	...
	if(myrank_mpi==0)																				// only the process with rank 0 will do this
	{
		...
		fu=fopen("Data/U_nu0A0.2k0.5Nx32Lx12.5664Nv24Lv5.25SpectralN8dt0.01nT2000_nu0_SpecReflec_T20.dc","r"); 		// set fu to be a file with the name U_nu0.02A0.5k0.785398Nx48Lx8Nv48Lv4.5SpectralN24dt0.004nT500_non_nu002_2.dc, stored in the directory Data, and set the file access mode of fu to r (which means the file must exist already and will be read from)
		...
		float NoSol = (float)tp/(size*7*sizeof(double));											// calculate the number of solutions of U that must be in the file
		printf("no. of set of U = %f \n", NoSol);													// display in the output file the number of solutions of U that must be in the file

		if(tp%(size*7) != 0)																		// check that the number of elements read was a multiple of 6*size
		{
		...
		{
			fseek(fu, tp - size*7*sizeof(double), SEEK_SET);										// find the last solution that was printed in the file
			fread(&U,sizeof(double),size*7,fu);														// store the contents of the final solution in the file fu in U, expecting 6*size many entries of datatype double
		}
	...
	while(t < nT) 																					// if t < nT (i.e. not yet reached the final timestep), perform time-splitting to first advect the particle through the collisionless step and then perform one space homogeneous collisional step)
	{
		...
		if(nu > 0.)
		{
			...
			for(l=chunk_Nx*myrank_mpi;l<chunk_Nx*(myrank_mpi+1) && l<size_x;l++) 						// divide the number of discretised space points equally over the number of MPI processes, so that each process receives a different chunk of space to work on
			{
				#ifdef FullandLinear																// only do this if FullandLinear was defined
			...
			if(myrank_mpi == 0) 																	// only the process with rank 0 will do this
			{
				...
				for(l=0;l<chunk_Nx;l++) 															// cycle through all space-steps stored in the first chunk of the space interval (which is stored on the process with rank 0)
				{
					for(k=0;k<size_v;k++)															// cycle through all size_v (= Nv^3) many velocity-steps (which will exist for each space-step)
					{
						...
						U[k_v*7+0] = Utmp_coll[k_v*5];												// set the 6*k_v-th entry of U to the 5*k_v-th entry of Utmp_coll
						U[k_v*7+6] = Utmp_coll[k_v*5+4]; 											// set the (6*k_v + 5)-th entry of U to the (5*k_v + 4)-th entry of Utmp_coll
						U[k_v*7+3] = Utmp_coll[k_v*5+1];  											// set the (6*k_v + 2)-th entry of U to the (5*k_v + 1)-th entry of Utmp_coll
						U[k_v*7+4] = Utmp_coll[k_v*5+2];	 										// set the (6*k_v + 3)-th entry of U to the (5*k_v + 2)-th entry of Utmp_coll
						U[k_v*7+5] = Utmp_coll[k_v*5+3];											// set the (6*k_v + 4)-th entry of U to the (5*k_v + 3)-th entry of Utmp_coll
					}
				}
				...
				for(i=1;i<nprocs_Nx;i++)															// store the DG coefficients of the current solution in U that were calculated by the remaining processes (with ranks i = 1, 2, ..., nprocs_Nx-1) for their corresponding chunk of space
				{
					...
					{
						if((chunk_Nx*i+l)<size_x)														// ensure that the space-step being dealt with exists
						{
							for(k=0;k<size_v;k++)													// cycle through all size_v (= Nv^3) many velocity-steps (which will exist for each space-step)
							{
								...
								U[k_v*7+0] = output_buffer[k_local*5];								// set the 6*k_v-th entry of U to the 5*k_local-th entry of Utmp_coll
								U[k_v*7+6] = output_buffer[k_local*5+4];							// set the (6*k_v + 5)-th entry of U to the (5*k_local + 4)-th entry of Utmp_coll
								U[k_v*7+3] = output_buffer[k_local*5+1];  							// set the (6*k_v + 2)-th entry of U to the (5*k_local + 1)-th entry of Utmp_coll
								U[k_v*7+4] = output_buffer[k_local*5+2];	 						// set the (6*k_v + 3)-th entry of U to the (5*k_local + 2)-th entry of Utmp_coll
								U[k_v*7+5] = output_buffer[k_local*5+3];							// set the (6*k_v + 4)-th entry of U to the (5*k_local + 3)-th entry of Utmp_coll
							}
						}
					}
				}
